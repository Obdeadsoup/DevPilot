...
flowchart LR
    ...
    A -->|gRPC ExecuteTool| T[Java Controlled Tool Gateway]
...
5. 准备 Project/Task/Activity 后，显式选择 Repository 与 Branch 再发起 Agent Run，观察 Tool lifecycle、SSE terminal 与历史权威状态；`task.create` 会显示固定 Proposal，人工批准或拒绝后从 Checkpoint 继续。
