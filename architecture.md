...
devpilot-agent           Agent 集成边界：Browser API、RBAC、Run 投影、RPC、只读 Tool Gateway
```
...
API Credential与 Webhook Secret 分别由独立白名单 Reference Provider/Resolver 提供。前者只供 REST
...
Python agent-service → DevPilotToolGateway gRPC → Java devpilot-agent
        → run-bound delegation + RBAC → 三个只读工具
